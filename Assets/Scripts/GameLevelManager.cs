...
public class GameLevelManager : MonoBehaviour
{
    ...
    public void SetMoveItemSlot_Finished(ItemTile _itemTile)
    {
        ...
        if (listCheckMatch3Slots.Count == 3)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                DailyQuest.UpdateTilesMatched(1);
    ...
    public List<ItemTileSlot> FindMatch3_ItemTile_Slots(ItemTile _itemTile)
    {
        ...
        // This will store the tiles making up the triplet
        List<ItemTile> tripletTiles = new List<ItemTile>();
        ...
        for (int i = 0; i < listItemSlots.Count; i++)
        {
            if (listItemSlots[i].itemTile.itemData.itemType == _itemTile.itemData.itemType)
            {
                ...
                tripletTiles.Add(listItemSlots[i].itemTile);

                if (listCheckMatch3_ItemTileSlot.Count == 3)
                {
                    // Check if the itemData matches 'itemData2'
                    if (_itemTile.itemData.itemType == Config.ITEM_TYPE.ITEM_2)
                    {
                        // Log or store the triplet tiles information
                        Debug.Log($"Triplet Found: {tripletTiles[0].itemData.itemType}, {tripletTiles[1].itemData.itemType}, {tripletTiles[2].itemData.itemType}");
                        Debug.Log("Gotttt it");

                        // Store this information for further use
                        StoreTripletInformation(tripletTiles);
                    }
    ...
    // Store the triplet information for later use
    private void StoreTripletInformation(List<ItemTile> tripletTiles)
    {
        Config.UpdateTileJoinVariable();
    }
    #endregion
